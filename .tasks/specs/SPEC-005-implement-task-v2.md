...
- Guardrail hooks for v2. Guardrails get rebuilt from the ground up later; during development the human disables this repo's hooks locally.
...
v2 ignores the following keys. The key notes mark them as v1-only, to be removed at cutover:
...
This repo's `PreToolUse` hooks deny pushing task work to `default_branch`, which `merge changes`
requires. Disable them locally before running v2, with `{"disableAllHooks": true}` in
`.claude/settings.local.json` (gitignored). While they're enabled, a denial counts toward
`guardrail_denial`.
...
- `CLAUDE.md`'s Guardrails section notes v2's local squash-merge and push to `main`, and the
  local hook disable.
- `.tasks/config.md` key notes say which skill reads each key, and mark the v1-only keys.
