...
These epics do not exist as files yet; a later pass creates them via `plan-feature` / `add-task`.
Sequencing follows the analysis doc: data model first, then tooling, then skills, then dogfood.

| Epic | Scope |
|---|---|
| EPIC-001 | **Data model & templates** — frontmatter schemas, spec/epic/task templates, PR template, `guidelines.md`, `config.md` schema |
| EPIC-002 | **`sync` script** — region regeneration, epic-status derivation, epic panel, `next-id`, blocked reconciliation, archiving, `check` mode, idempotency tests |
| EPIC-003 | **`init` + `add-task` skills** |
| EPIC-004 | **`implement-task` skill** — four phases, resumable, bail-out |
| EPIC-005 | **`refine-backlog` + `plan-feature` skills** |
| EPIC-006 | **Dogfood & harden** — migrate this repo's board onto the toolkit, wire `sync check` into CI |
