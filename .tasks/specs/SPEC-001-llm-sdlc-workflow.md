...
An earlier design analysis (now superseded by this spec) critiqued the workflow sketched in
`README.md` and reached firm conclusions, but as feedback on a design rather than something
buildable: no schemas to code against, no derivation rules, no scope line. Separately, the
workflow had no concept of an **epic** — a grouping layer above the task — which the user relies
on in Jira (Epic → Task, plus an epic panel on the board).

This spec turns that analysis into a PRD for a v1 toolkit, and adds epic tracking.
...
- **Why `implement-task` stops before merging, rather than running end to end** → the original
  design did phases 1–4 in one uninterrupted run, including auto-merge to `default_branch`. That
  was rejected: auto-merging past the human's own review defeats the purpose of opening a PR at
  all, even solo. Phase 3 always ends in a hard STOP; phase 4 only observes the human's merge.
...
| Epic | Status | Progress |
|---|---|---|
| EPIC-001 | in-progress | 35/36 done |
<!-- END:epics -->
